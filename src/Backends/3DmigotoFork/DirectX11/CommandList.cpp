		...
		const wstring *ini_namespace)
{
	...
	// Clearing is intentionally a modifier for description-only allocation.
	// Allowing it with a full copy would immediately overwrite the copied data,
	// while allowing it with a reference would mutate a game-owned resource.
...
static bool RecreateCompatibleResource(
		wstring *ini_line,
		ResourceCopyTarget *dst, // May be NULL
		...
		UINT *buf_dst_size)
{
	...
	// A non-NULL return means the destination switched to a newly created
	// or pooled resource because its description or device changed. Callers
	// can use this to initialise temporal resources without clearing them on
	// ordinary frames where the same compatible resource is retained.
	return res != NULL;
}
...
void ResourceCopyOperation::run(CommandListState *state)
{
	...
	if (options & ResourceCopyOptions::COPY_MASK) {
		bool resource_recreated = RecreateCompatibleResource(&ini_line, &dst, src_resource,
		...
		if (resource_recreated && (options & ResourceCopyOptions::CLEAR_ON_CREATE)) {
			ClearViewCommand clear_command;
			ID3D11View *clear_view = clear_command.create_best_view(
				dst_resource, state, stride, offset, format, buf_dst_size);
			if (clear_view) {
				COMMAND_LIST_LOG(state, "  clearing newly created destination resource\n");
				clear_command.clear_unknown_view(clear_view, state);
				clear_view->Release();
			} else {
				COMMAND_LIST_LOG(state, "  unable to clear newly created destination resource\n");
			}
		}
