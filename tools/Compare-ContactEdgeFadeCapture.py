...
def main():
    ...
    p.add_argument("--capture-suffix", choices=("", "-v2"), default="")
    a = p.parse_args()
    ...
    dirs = {i: a.fork / "artifacts" / f"fade-capture-{i}{a.capture_suffix}" for i in (0, 1, 9)}
    ...
    for light in (50, 38, 54, 20, 52):
        for enabled in (0, 1):
            ...
            assert (values[9] >= values[1] - 2e-6).all(), "Wider fade darkened a sample"
            if not enabled:
    ...
    output.mkdir()
    report = {"status": "passed-captured-visibility-comparison", "zeroWidthBitIdenticalFiles": baseline_matches,
