...
{
	return ((UE4FXRemoteInput::CheckVirtualKeyState(vkey) < 0) ^ invert);
}
...
bool DispatchInputEvents(HackerDevice *device)
{
	...
	int j;
	bool remote_input_frame = UE4FXRemoteInput::BeginDispatchFrame();
	wchar_t remote_status_message[UE4FXRemoteInput::status_message_characters] = {};
	if (UE4FXRemoteInput::TryConsumeStatusMessage(remote_status_message, ARRAYSIZE(remote_status_message)))
		LogOverlayW(LOG_WARNING, L"Codex: %s\n", remote_status_message);
	if (!remote_input_frame && !CheckForegroundWindow())
		return false;
