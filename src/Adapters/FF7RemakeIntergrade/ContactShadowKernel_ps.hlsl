...
#if defined(REDX11_CONTACT_SOFTNESS_EXPERIMENT)
// FXC cannot prove that the donor adapter's call-scoped static bridge is
// initialized across repeated inlined rays. Every bridge field is assigned at
// the start of Redx11TraceRebirthContactShadow; suppress only that false alarm.
...
float Redx11RemakeContactPixel(float3 input, bool traceEnabled, out bool receiverValid)
{
    ...
    #if defined(REDX11_CONTACT_SOFTNESS_EXPERIMENT)
    // Page Up experiment: only material specialization 1 (the 62b route when
    // installed in isolation) may use the virtual area emitter. Page Down and
    // row 31 remain the sole master gate for all injected contact lighting.
    float4 softnessControl=ContactControl.Load(int2(28,0));
    float emitterRadius=max(softnessControl.y,0.0f);
    float visibility=(softnessControl.x==1.0f && material==1u && emitterRadius>0.0f)
        ? Redx11TraceRebirthSoftContactPositiveRadius(translated,normal,direction,lightDistance,
            inverseRadius,jitter,material,1.5f,view,settings,emitterRadius)
        : Redx11TraceRebirthContactShadow(translated,normal,direction,lightDistance,
            inverseRadius,jitter,material,1.5f,view,settings);
    #else
