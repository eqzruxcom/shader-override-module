...
$fork=Join-Path $repo 'artifacts/contact-edge-profile-development-20260831-v1'
$candidateRoot=Join-Path $fork 'artifacts/profile-candidate-v1'
...
$evidence=@()
foreach($percent in @(0,1)){
    $path=Join-Path $fork "artifacts/profile-interface-$percent-v2/manifest.json"
    $check=Read-Json $path
    $iterations=if($percent -eq 1){8}else{1}
    if($check.cutoffPercent -ne $(if($percent -eq 1){.5}else{0}) -or $check.fullStrengthPercent -ne $(if($percent -eq 1){4}else{0})){throw 'Wrong tested profile'}
    ...
    foreach($variant in $candidate.variants){
        ...
        $fixture=if($percent -eq 1){$variant.repeatedFixture}else{$variant.injectionFixture}
...
$mathPath=Join-Path $repo 'artifacts/contact-edge-profile-tests-20260831-v2/manifest.json'
$math=Read-Json $mathPath
if($math.status -ne 'passed-left-cutoff-profile' -or $math.cutoffPercent -ne .5 -or $math.fullStrengthPercent -ne 4 -or $math.mathChecks -ne 1056 -or $math.rayChecks -ne 1536 -or $math.qualityGatePassed -ne $false){throw 'Missing left-only numeric evidence'}
Assert-Sources $fork $math
Assert-Hash (Join-Path $repo 'tools/Test-ContactEdgeProfile.ps1') $math.scriptSha256
$capturePath=Join-Path $repo 'artifacts/contact-edge-profile-capture-comparison-20260831-v2/manifest.json'
$capture=Read-Json $capturePath
if($capture.status -ne 'passed-captured-profile-comparison' -or $capture.cutoffPercent -ne .5 -or $capture.fullStrengthPercent -ne 4 -or $capture.zeroWidthBitIdenticalFiles -ne 20 -or $capture.qualityGatePassed -ne $false){throw 'Missing captured comparison'}
Assert-Hash (Join-Path $repo 'tools/Compare-ContactEdgeProfileCapture.py') $capture.scriptSha256
foreach($item in $capture.evidence){
    $evidencePath=[IO.Path]::GetFullPath($item.path)
...
foreach($percent in @(0,1)){Assert-Sources $fork (Read-Json (Join-Path $fork "artifacts/profile-capture-$percent-v2/manifest.json"))}
...
$priorRoot=Join-Path $repo 'artifacts/generated-runtime/FF7RemakeIntergradeContactLeftEdgeFade-live-v1'
...
$lines=@('; LEFT-only contact-darkness fade; experimental, not release-ready.',
    '; Starts with working donor contact shadows ON and fade 0%.',
    '; 0 = no fade. 1 = zero at 0.5%, full at 4%. 2..9 = old width-only presets.',
    '; Home toggles all added contact shadows. No native shadows, crop or FOV changed.',
    '[Constants]','global $ue4fx_contact_edge_profile_v2 = 1','global $ue4fx_contact_edge_width_v2 = 0','global $ue4fx_contact_edge_cutoff_v2 = 0','',
    '[KeyUE4FXContactHome]','key = no_modifiers VK_HOME','type = cycle','smart = true',
    '$ue4fx_contact_edge_profile_v2 = 0, 1','')
foreach($percent in 0..9){
    $value=if($percent -eq 1){'0.04'}else{([double]$percent/100).ToString('0.00',[Globalization.CultureInfo]::InvariantCulture)}
    $cutoff=if($percent -eq 1){'0.005'}else{'0'}
    $lines+=@("[KeyUE4FXContactEdge$percent]","key = no_modifiers $percent",'$ue4fx_contact_edge_profile_v2 = 1',('$ue4fx_contact_edge_width_v2 = '+$value),('$ue4fx_contact_edge_cutoff_v2 = '+$cutoff),'')
}
foreach($hash in $hashes){$lines+=@("[ShaderOverrideUE4FXContact$hash]","hash = $hash",'x29 = $ue4fx_contact_edge_width_v2','y29 = $ue4fx_contact_edge_cutoff_v2','x31 = $ue4fx_contact_edge_profile_v2','y31 = -1','z31 = 1','w31 = 100','')}
...
foreach($percent in 0..9){
    ...
    $expectedWidth=if($percent -eq 1){'0.04'}else{([double]$percent/100).ToString('0.00',[Globalization.CultureInfo]::InvariantCulture)}
    $expectedCutoff=if($percent -eq 1){'0.005'}else{'0'}
    if(-not $section.Success -or $section.Groups[1].Value -notmatch ('(?m)^key = no_modifiers '+$percent+'\r?$') -or
        $section.Groups[1].Value -notmatch ('(?m)^\$ue4fx_contact_edge_width_v2 = '+[regex]::Escape($expectedWidth)+'\r?$') -or
        $section.Groups[1].Value -notmatch ('(?m)^\$ue4fx_contact_edge_cutoff_v2 = '+[regex]::Escape($expectedCutoff)+'\r?$') -or
        $section.Groups[1].Value -notmatch '(?m)^\$ue4fx_contact_edge_profile_v2 = 1\r?$'){throw 'Direct numeric preset mismatch'}
}
if(([regex]::Matches($controlText,'(?m)^x29 = \$ue4fx_contact_edge_width_v2\r?$')).Count -ne 5 -or
    ([regex]::Matches($controlText,'(?m)^y29 = \$ue4fx_contact_edge_cutoff_v2\r?$')).Count -ne 5 -or
    $controlText -notmatch '(?m)^global \$ue4fx_contact_edge_width_v2 = 0\r?$' -or
    $controlText -notmatch '(?m)^global \$ue4fx_contact_edge_cutoff_v2 = 0\r?$'){throw 'Wrong fade binding or initial profile'}
...
$manifest.mode='rebirth-contact-left-edge-profile-experiment-v2'
$manifest.authorization='User requested the preferred left-edge opacity fade test, starting at 4%, while they work; no game keys'
$manifest.testProfile=@{key='1';cutoffPercent=.5;fullStrengthPercent=4;curve='smoothstep';temporalHistory=$false;otherKeys='0=no fade; 2..9=old width-only percentages'}
...
[pscustomobject]@{State='left-edge-profile-prepared';Installed=$installed;Output=$output;Default='Contact ON, fade OFF';Keys='0=no fade; 1=0.5%-zero to 4%-full; 2..9=old width-only percentages; Home=contact toggle';ReloadSent=$false}
