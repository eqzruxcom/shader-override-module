...
- The private temporal indirect-light candidate now matches native c473 moving/static reprojection and avoids finished-scene feedback. Live testing showed the visible-source angle cutoff remained mostly unchanged, so it was not graduated. An offline-verified Page Up diagnostic now isolates 4/8/16 angular trace slices while F2 remains the candidate master and F10/Page Down remain untouched. Its guarded twelve-file live transition has also passed staging, drift-rejection, and byte-exact rollback tests in a disposable game tree; it remains uninstalled and live-unproven. See [the angular-coverage diagnostic](docs/intergrade-ssgi-angular-coverage-diagnostic.md).
- The next native-light coverage gate now has a family-derived, read-only capture pack. It targets the exact accepted `3+1+1` tiled-light family plus its classifier, adds no keys or rendering commands, and has passed fresh generation, atomic staging, drift rejection, and exact removal in a disposable tree. It remains uninstalled while the game is running. See [the native light-profile capture](docs/intergrade-native-light-profile-capture.md).
