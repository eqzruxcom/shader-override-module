...
if($SoftnessExperiment -and ($Implementation -ne 'Rebirth' -or $Reconstruction -eq 'RecomputeQuad')) {throw 'The isolated softness experiment requires the raw or shared-quad Rebirth implementation.'}
...
$compileArguments=@('/nologo','/Ges','/Gis','/WX','/O3','/D',"REDX11_CONTACT_SAMPLES=$Samples")
if($SoftnessExperiment){$compileArguments+=@('/D','REDX11_CONTACT_SOFTNESS_EXPERIMENT=1')}
$compileArguments+=@('/T',$profile,'/E','main','/Fo',$kernelBinary,$kernelSource)
Run-ContactTool $FxcPath $compileArguments (Join-Path $validation 'kernel-compile.log')
...
$records=@(foreach ($v in $variants) {
    ...
    # Keep the original comment header for the emitted shader; matching still
    # uses normalized instructions. Do not discard the signatures/provenance.
    $originalLines=@(Get-Content -LiteralPath (Join-Path $validation ($v.Hash+'-original.asm')))
    $profileLine=[array]::IndexOf($originalLines,'cs_5_0')
    if($profileLine -lt 1){throw 'Expected original disassembly header.'}
    $originalHeader=$originalLines[0..($profileLine-1)]
    if(@($originalHeader | Where-Object {$_.Trim() -and -not $_.TrimStart().StartsWith('//')}).Count){throw 'Unexpected non-comment header.'}
    ...
    [IO.File]::WriteAllText($candidateAsm,(($originalHeader+$after.ToArray()) -join "`n")+"`n",$utf8)
...
$report=[ordered]@{
    ...
    implementation=$Implementation;reconstruction=$Reconstruction;softnessExperiment=[bool]$SoftnessExperiment
    ...
    sources=@(foreach($path in @('src/Effects/Lighting/ContactShadowCommon.hlsl','src/Effects/Lighting/ContactShadows.hlsl','src/Effects/Lighting/RebirthContactShadows.hlsl','src/Effects/Lighting/RebirthContactSoftness.hlsl','src/ThirdParty/ShaderInjector/RebirthContactRay.hlsl','src/ThirdParty/ShaderInjector/RebirthContactNoise.hlsl','src/ThirdParty/ShaderInjector/provenance.json','src/Adapters/FF7RemakeIntergrade/ContactShadowKernel_ps.hlsl','src/Adapters/FF7RemakeIntergrade/RebirthContactShadowKernel_ps.hlsl')) {
    ...
    control=$(if($SoftnessExperiment){'IniParams row31: master/light/strength/ray length; row28: x=softness enabled, y=virtual emitter radius in world units'}else{'IniParams row31: x=enabled(1), y=light index(-1 means all), z=strength, w=ray length'})
