...
static void TakeScreenShot(HackerDevice *wrapped, void *private_data)
{
	...
	// The historical callback below only asks NVAPI for a stereo capture. That
	// silently produces no image when stereo is disabled, which makes it
	// unsuitable for the remote, no-focus test channel. Always save the current
	// mono back buffer as well. The tick count keeps repeated captures distinct,
	// and the existing helper uses DirectXTK/WIC without activating the window.
	SimpleScreenShot(wrapped, GetTickCount64(), "screenshot");
...
static void AnalyseFrame(HackerDevice *device, void *private_data)
{
	...
	// Soft-disabled hunting intentionally hides shader/IB/VB selection effects,
	// but frame analysis is read-only telemetry and remains safe and useful.
	// Keep the stock refusal only for fully disabled hunting.
...
{
	// Clearing cached selections is valid while hunting is soft-disabled.
	// Only hard-disabled mode lacks the supporting hunting state.
	if (G->hunting == HUNTING_MODE_DISABLED)
		return;

	LogInfo("> Hunting selections cleared\n");
	EnterCriticalSectionPretty(&G->mCriticalSection);
